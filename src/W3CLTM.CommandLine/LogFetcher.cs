...
namespace IISLogParser.CommandLine
{
    ...
    {
        public virtual IEnumerable<FileInfo> GetLogFiles(string path, string lastFileName,
                                                         bool fallbackReturnAll = false)
        {
            IOrderedEnumerable<FileInfo> files = new DirectoryInfo(path).GetFiles().OrderByDescending(x => x.Name);
        ...
        {
            return
                new StreamReader(new FileStream(file.FullName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite),
                                 Encoding.Default).ReadLines();
        }
    }
}