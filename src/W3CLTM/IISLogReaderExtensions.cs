...
namespace IILogReader
{
    public static class IISLogReaderExtensions
    {
        ...
        public static void BootStrapFromSource(this W3CLogReader w3c, Configuration.Configuration.source source)
        {
            ...
            source.conversions.ForEach(
                conversion =>
                w3c.Convert(conversion.elementName).ToType(
                    Type.GetType("System." + (conversion.type == "int" ? "Int32" : conversion.type))));
            source.composites.ForEach(
                composite => w3c.AddComposite.Named(composite.elementName).FromElements(composite.elements));
            source.multiColumnElements.ForEach(mce => w3c.MultiColumnElement(mce.elementName, mce.colSpan));
            source.staticElements.ForEach(
                staticElement => w3c.AddStaticElement(staticElement.elementName, staticElement.elementValue));
        ...
        public static void ApplyTemplate(this Configuration.Configuration.source source, JObject job)
        {
            ...
            if (job.Properties().Any(x => x.Name == "conversions"))
                job["conversions"].Children().ForEach(
                    x =>
                    source.conversions.Add(
                        JsonConvert.DeserializeObject<Configuration.Configuration.source.conversion>(x.ToString())));

            if (job.Properties().Any(x => x.Name == "composites"))
                job["composites"].Children().ForEach(
                    x =>
                    source.composites.Add(
                        JsonConvert.DeserializeObject<Configuration.Configuration.source.composite>(x.ToString())));

            if (job.Properties().Any(x => x.Name == "muliColumnElements"))
                job["multiColumnElements"].Children().ForEach(
                    x =>
                    source.multiColumnElements.Add(
                        JsonConvert.DeserializeObject<Configuration.Configuration.source.mulitColumnElement>(
                            x.ToString())));

            if (job.Properties().Any(x => x.Name == "staticElements"))
                job["staticElements"].Children().ForEach(
                    x =>
                    source.staticElements.Add(
                        JsonConvert.DeserializeObject<Configuration.Configuration.source.staticElement>(x.ToString())));

            if (job.Properties().Any(x => x.Name == "aliases"))
                job["aliases"].Children().ForEach(
                    x =>
                    source.aliases.Add(
                        JsonConvert.DeserializeObject<Configuration.Configuration.source.elementAlias>(x.ToString())));

            if (job.Properties().Any(x => x.Name == "templates"))
                job["templates"].Children().ForEach(x => source.templates.Add(x.ToString()));
        }

        public static void Overlay(this Configuration.Configuration.source baseSource,
                                   Configuration.Configuration.source overlay)
        {
        ...
        public static Configuration.Configuration.source ProcessSource(
            this Configuration.Configuration.source initialSource)
        {
            var sourceBase = new Configuration.Configuration.source();

            if (initialSource.templates.Any())
                initialSource.templates.ForEach(
                    x =>
                    sourceBase.Overlay(
                        JsonConvert.DeserializeObject<Configuration.Configuration.source>(File.ReadAllText(x))));
        ...
        public static IEnumerable<string> SplitValues(this string line)
        {
            //@"""(?<value>.+?)""|(?<value>[^\s""]+)"
            //@"\s?""(?<value>.+?)""\s|(?<value>[^\s]+)"
            var regexObj = new Regex(@"\s?""(?<value>.+?)""\s|(?<value>[^\s]+)");
        ...
        public static IEnumerable<IEnumerable<T>> InBatchesOf<T>(this IEnumerable<T> source, int max, Logger logger)
        {
            ...
            else
            {
                ...
                foreach (T item in source)
                {
                    ...
                    {
                        sw.Stop();
                        logger.Log("{0} elements in {1}ms. {2}p/s".Frmat(max, sw.ElapsedMilliseconds,
                                                                         sw.ElapsedMilliseconds > 0
                                                                             ? (max/sw.ElapsedMilliseconds)*1000L
                                                                             : 0L));
                        yield return toReturn;
                        toReturn = new List<T>(max);
                    }
                ...
                if (toReturn.Any())
                {
                    logger.Log("{0} elements in {1}ms. {2}p/s".Frmat(toReturn.Count, sw.ElapsedMilliseconds,
                                                                     sw.ElapsedMilliseconds > 0
                                                                         ? (toReturn.Count/sw.ElapsedMilliseconds)*1000L
                                                                         : 0L));
                    yield return toReturn;
                }
            }
