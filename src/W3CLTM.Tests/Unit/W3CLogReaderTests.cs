...
2012-07-23 12:57:34 GET /foo/bar.html";
            ...
            fields = new string[] { "time", "cs-method", "cs-uri" };
            values = new List<string[]>();
            values.Add(new string[] { "00:34:23", "GET", "/foo/bar.html" });
            values.Add(new string[] { "12:21:16", "GET", "/foo/bar.html" });
            values.Add(new string[] { "12:45:52", "GET", "/foo/bar.html" });
            values.Add(new string[] { "12:57:34", "GET", "/foo/bar.html" });
            values.Add(new string[] { "2012-07-23", "00:34:23", "GET", "/foo/bar.html" });
            values.Add(new string[] { "2012-07-23", "12:21:16", "GET", "/foo/bar.html" });
            values.Add(new string[] { "2012-07-23", "12:45:52", "GET", "/foo/bar.html" });
            values.Add(new string[] { "2012-07-23", "12:57:34", "GET", "/foo/bar.html" });
            ...
            logger.When(x=>x.Log(Arg.Any<string>(),Arg.Any<Exception>()))
                .Do(x=>
                        {
                            x.Args()[0].WriteLine();
                            throw new Exception("Shouldn't be throwing exceptions:{0}".Frmat(x.Args()[0]),(Exception)x.Args()[1]);
                        });
        ...
        public void Should_Return_Fields(string line)
        {
            ...
            Enumerable.SequenceEqual(line.SplitFields(), new string[] { "time", "cs-method", "cs-uri" }).ShouldBe(true);
        }
        ...
        {
            line.SplitValues().ForEach(x => Console.Write(x + " "));
            Enumerable.SequenceEqual(line.SplitValues(), new string[] { "hello newman", "you bastard, I'll kill you", "die!" }).ShouldBe(true);
        }
...
//        }
        ...
        {
            IEnumerable<string> lines = file.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            w3cReader.Process(lines);
            Enumerable.SequenceEqual(w3cReader.Fields, new string[] { "time", "cs-method", "cs-uri" });
        }
        ...
        {
            IEnumerable<string> lines = file.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            var entries = w3cReader.Process(lines).ToList();
            ...
            entries.ForEach((x, y) => Enumerable.SequenceEqual(x, expextedEntries[y]).ShouldBe(true));
        }

        [Test]
        public void Should_Process_Entries_WithMultiple_Field_Directives()
        {
            IEnumerable<string> lines = (file + Environment.NewLine + file2).Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            var entries = w3cReader.Process(lines).ToList();
            var expextedEntries = new List<IDictionary<string, object>>();
            var dic1 = new Dictionary<string, object>();
            fields.ForEach((x, y) => dic1.Add(x, values[0][y]));
            ...
            fields.ForEach((x, y) => dic4.Add(x, values[3][y]));
            expextedEntries.Add(dic4);
            var fields2 = (new string[] { "date" }).Union(fields);
        ...
        public void Should_Remember_Line_Position_When_Starting_ElseWhere()
        {
            var fields2 = (new string[] { "date" }).Union(fields);
            IEnumerable<string> lines = (file + Environment.NewLine + file2).Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            var entries = w3cReader.FromLine(10).WithFields(fields2).Process(lines).ToList();
        ...
        {
            IEnumerable<string> lines = file.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            var entries = w3cReader.Limit(2).Process(lines).ToList();
            var expextedEntries = new List<IDictionary<string, string>>();
        ...
        {
             IEnumerable<string> lines = new string[] { "2012-07-23 09:57:31 GET 400" };
             w3cReader.Process(lines).ToList();
        }

        public void Should_Combine_Columns_To_New_Column()
        {
            var fields = new string[] {"date", "time", "method", "status"};
            IEnumerable<string> lines = new string[] { "2012-07-23 09:57:31 GET 400" };
            ...
            var entries = w3cReader.Process(lines).ToList();
            Enumerable.First<IDictionary<string, object>>(entries).ForEach(y =>
                          {
                              Console.WriteLine("{0} {1}", y.Key, y.Value);
                          });
            Enumerable.First<IDictionary<string, object>>(entries).ForEach(x => Console.WriteLine("{0} {1}", x.Key, x.Value));
            Enumerable.First<IDictionary<string, object>>(entries)["LogTime"].ShouldBe("2012-07-23 09:57:31");
        }
        ...
        {
            var fields = new string[] { "date", "time", "method", "status","blarg"};
            IEnumerable<string> lines = new string[] { "2012-07-23 09:57:31 GET 400 -" };
            w3cReader.WithFields(fields);
        ...
        public void Should_Add_Specified_Static_Elements()
        {
            ...
            Enumerable.First<IDictionary<string, object>>(entries).ForEach(x => Console.WriteLine("{0} {1}", x.Key, x.Value));
            Enumerable.First<IDictionary<string, object>>(entries).Count.ShouldBe(5);
            Enumerable.First<IDictionary<string, object>>(entries)["static"].ShouldBe("some shit");
        }
        ...
        {
             var fields = new string[] { "date", "Endtime-UTC", "method", "status"};
            IEnumerable<string> lines = new string[] { "2012-07-23 2012-07-23 09:57:31 GET 400" };
            w3cReader.WithFields(fields);
        ...
        {
             var fields = new string[] { "date", "Endtime-UTC", "method", "status"};
            IEnumerable<string> lines = new string[] { "2012-07-23 - GET 400" };
            w3cReader.WithFields(fields);
        ...
        {
            var fields = new string[] { "date", "time", "method", "status"};
            IEnumerable<string> lines = new string[] { "2012-07-23 09:57:31 - 400" };
            w3cReader.WithFields(fields);
        ...
        public void Should_Handle_Conversion_Fail_Gracefully()
        {
            ...
            var fields = new string[] { "date", "time", "method", "status"};
            IEnumerable<string> lines = new string[] { "2012-07-23 09:57:31 - booger" };
